...
const getBookings = async (req, res) => {
  try {
    ...
    const userBookings = await Booking.find({ user: req.session.user.id })
      .populate('user', 'name')
      .lean();
...
const cancelBooking = async (req, res) => {
  try {
    ...
    if (!booking) {
      return res.redirect('/bookings?error=Prenotazione non trovata');
    }

    // Verifica che la prenotazione appartenga all'utente corrente
    if (booking.user.toString() !== req.session.user.id) {
      return res.redirect('/bookings?error=Non puoi cancellare questa prenotazione');
    }

    // Verifica che non sia il giorno della prenotazione
    const today = DateTime.now().startOf('day');
    const bookingDate = DateTime.fromJSDate(booking.bookingDate).startOf('day');

    if (today.hasSame(bookingDate, 'day')) {
      return res.redirect('/bookings?error=Non puoi cancellare una prenotazione per oggi');
    }

    // Cancellazione della prenotazione
    await Booking.findByIdAndDelete(id);

    res.redirect('/bookings?success=Prenotazione cancellata con successo');
  } catch (error) {
    res.redirect('/bookings?error=Errore durante la cancellazione della prenotazione');
  }
};
